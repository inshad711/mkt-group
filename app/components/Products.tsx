...
const ProductImageArea = ({ product }: { product: Product }) => {
    return (
        <div className="relative w-full h-72 rounded-3xl overflow-hidden group bg-gray-900">
            {/*  Image  */}
        ...
        <div className="bg-gray-100 p-8 font-sans">
            ...
            <div className="templateContainer mb-12 flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">
                        PRODUCTS</h1>
                    <p className="text-gray-500 mt-2">Curated for your specific workout plan.</p>
                </div>
            ...
            <div className="templateContainer grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {products.map((product) => (
                    <div
                        key={product.id}
                        className="bg-white rounded-[2rem] p-4 shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col"
                    >
                        {/* Card Header */}
